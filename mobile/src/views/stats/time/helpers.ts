...
// Tap-toggle tooltip state — mobile has no hover, so every ported grid cell toggles
// this on tap instead (same cell tapped again clears it, via the `key` field).
export interface TapHover { x: number; y: number; label: string; sub: string; key: string; }
